...
*/
open class SobrCameraView: UIView, AVCaptureVideoDataOutputSampleBufferDelegate , AVCapturePhotoCaptureDelegate  {
    ...
    */
    open func setupCameraView() {
        ...
        //let aDevice = AVCaptureDevice.defaultDevice(withDeviceType: .builtInWideAngleCamera, mediaType: AVMediaTypeVideo, position: .back)

        let aDevice  : AVCaptureDevice? = AVCaptureDevice.default(.builtInWideAngleCamera, for: AVMediaType.video, position: .back)
        if aDevice == nil {
            return
        }
        
        self.captureSession.beginConfiguration()
        self.captureDevice = aDevice!
...
//        dataOutput.videoSettings = [kCVPixelBufferPixelFormatTypeKey: kCVPixelFormatType_32BGRA]
        ...
        let connection = dataOutput.connections.first!
            //as! AVCaptureConnection
        connection.videoOrientation = .portrait
        
        if self.captureDevice!.isFlashAvailable {
            try! self.captureDevice?.lockForConfiguration()
            
            //self.AVCapturePhotoSettings.flashMode
